...
### A. Figma Conventions
You **must** follow the file, page, and token structure defined in `.agents/tools/figma.md`. Specifically:
* **Design System file:** All tokens, foundations, and shared components live in the Design System file specified in `figma.md`. This file is the single source of truth. When you create or modify tokens, variables, or components, you do so in this file and publish updates to the team library.
* **Feature files:** High-fidelity visual design for a specific feature belongs on the **Visual Design** page of the corresponding feature file. Approved screens are moved to the **Ready for Development** page.
* **Token structure:** Tokens are implemented as Figma Variables organized into Primitives, Semantics, and (when needed) Component-specific collections, following the hierarchy in `figma.md`.
* **Component organization:** Components follow the atomic hierarchy (Atoms → Molecules → Organisms), each on their own page in the Design System file.
* **Library publishing:** After any token or component change, re-publish the library with a descriptive update note.
* **No detached instances:** Feature files consume the Design System library — components are never detached or overridden locally.

### B. The DRY Principle (Don't Repeat Yourself)
...
### C. Modular & Atomic Design
Components follow a strict hierarchy. **Tokens are global:** define or update the shared variables (colors, type scale, spacing, shadows, radii) only when establishing the baseline or when the scoped component justifies a token change—not fresh "system design" on every component. For the **current session's** component: use existing tokens and atoms where possible; add or adjust **only** what that component requires; compose molecules/organisms when the scoped unit is composite; reserve full templates/pages for when that template is explicitly the single scoped deliverable.

### D. Exhaustive Generation & States
...
* **Variants:** Define component variants using Figma's component properties (size, style, state) so developers can switch between them cleanly.
